...
void setup() {
  ...
  scale2.set_scale(scale_factor);       // 設定比例參數
...
void weight_update(){
  ...
  if(weight_diff<0) current_data[6] = 1;
  else current_data[6] = 2;
  weight_diff = abs(weight_diff);
  current_data[4] = int(weight_diff);
  current_data[5] = 100*weight_diff - 100*int(weight_diff);
